...
def train(loader: DataLoader, model: torch.nn.Module, criterion, optimizer: Optimizer, 
          epoch: int, device, print_freq, display=False):
...
def model_inference(base_classifier, loader, device, display=False, print_freq=1000):
    print_freq = 1000
    top1 = AverageMeter()
    top5 = AverageMeter()
...
def apply_aux_params(net, keep_masks, alpha):
    ...
    for layer, keep_mask in zip(prunable_layers, keep_masks):
        ...
        # Step 1: Set the masked weights to zero (NB the biases are ignored)
        # Step 2: Make sure their gradients remain zero
        layer.weight.data[keep_mask >= 0.] = alpha
        layer.weight.data[keep_mask < 0.] = -alpha
        layer.weight.register_hook(hook_factory(keep_mask))
